...
def get_student_result(roll_number, exam_type):
    """Fetches the student result from the database and calculates percentage correctly."""
    with connect_db() as conn:
        cursor = conn.cursor()
    ...
    # Consider only subjects with marks > 0 for percentage calculation
    valid_subjects = [marks for _, marks in result if marks > 0]
    total_subjects = len(valid_subjects)

    # Calculate percentage based on valid subjects (max marks per subject = 50)
    percentage = (sum(valid_subjects) / (total_subjects * 50)) * 100 if total_subjects > 0 else 0
